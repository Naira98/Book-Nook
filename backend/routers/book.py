...
@book_router.get("/by-interests", response_model=list[BookResponse])
async def get_books_by_interests(
    db: AsyncSession = Depends(get_db),
    current_user_id: int = Depends(get_user_id_via_session),
):
    """
    Returns books whose category, title, or description
    match the current user's interests.
    Interests are stored on the User model as a comma-separated string.
    """
    # Load user to get the raw interests string
    ...
    interests_lc = {s.lower() for s in interests}
    if not interests_lc:
        return []

    # Get all books
    all_books = await get_all_books(db=db)

    filtered: list[BookResponse] = []
    for b in all_books:
        # Extract fields (dict or Pydantic model support)
        title = None
        description = None
        category_name = None
        ...
        if isinstance(b, dict):
            ...
            category_name = b.get("category_name") or b.get("category")
        else:
            title = getattr(b, "title", None)
            description = getattr(b, "description", None)
            category_name = getattr(b, "category_name", None) or getattr(
                b, "category", None
            )

        # Convert to lowercase for case-insensitive matching
        combined_text = " ".join(
            str(x).lower() for x in [title, description, category_name] if x
        )

        # Check if any interest appears in combined_text
        if any(interest in combined_text for interest in interests_lc):
            filtered.append(b)
