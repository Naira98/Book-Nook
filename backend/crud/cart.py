...
async def add_to_cart_crud(
    db: AsyncSession, user_id: int, book_details_id: int, quantity: int
):
    existing_item = await db.execute(
        select(Cart).where(
            Cart.user_id == user_id, Cart.book_details_id == book_details_id
        )
    )
    ...
    if existing_item:
        ...
        cart_item = Cart(
            user_id=user_id, book_details_id=book_details_id, quantity=quantity
        )
        db.add(cart_item)
        await db.commit()
...
async def delete_cart_item_crud(db: AsyncSession, user_id: int, book_details_id: int):
