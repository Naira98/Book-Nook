...
const BookCard: React.FC<BookCardProps> = ({
  ...
  const canBorrow = hasBookDetails && book.book_details.some(detail =>
    detail.status === 'BORROW' && detail.available_stock > 0
  );
  const canPurchase = hasBookDetails && book.book_details.some(detail =>
    detail.status === 'PURCHASE' && detail.available_stock > 0
  );

  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden fade-in-up flex w-full max-w-3xl h-65 border border-gray-200">
      {/* Book Cover */}
...
//   return (
//     <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden fade-in-up flex flex-col w-[360px] h-[520px] border border-gray-200">
//       {/* Book Cover */}
...
//         <h3 className="font-semibold text-lg text-gray-900 mb-1 line-clamp-2 min-h-[48px]">
//           {book.title}
//         </h3>
...
//               <button
//                 onClick={() => onBorrow(book.id)}
//                 className="w-full bg-primary hover:bg-hover text-white px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 flex items-center justify-center space-x-1"
//               >
//                 <Clock className="w-4 h-4" />
//                 <span>Borrow</span>
//               </button>
...
//               <button
//                 onClick={() => onPurchase(book.id)}
//                 className="w-full bg-secondary hover:bg-yellow-500 text-white px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 flex items-center justify-center space-x-1"
//               >
//                 <ShoppingCart className="w-4 h-4" />
//                 <span>Add to Cart</span>
//               </button>
